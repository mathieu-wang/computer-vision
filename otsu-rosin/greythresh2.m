function level = greythresh2(I, T1)
%   Modified the graythresh method to add an optional threshold which
%   limits the range of levels we run the Otsu method on.
%
...
previous_thresh_value = 0;
counts = imhist(I,num_bins);

% Variables names are chosen to be similar to the formulas in
% the Otsu paper.
if nargin == 2
    previous_thresh_value_float = T1 * (num_bins-1);
    previous_thresh_value = ceil(previous_thresh_value_float);
    if previous_thresh_value == 0
        previous_thresh_value = 1; % handle case where every pixel is 0
    end
    counts = counts(previous_thresh_value:end);
    num_bins = num_bins - previous_thresh_value + 1;
end
...
if isfinite(maxval)
    ...
    level = (idx + previous_thresh_value - 1) / (old_num_bins - 1);
else
    level = 0.0;
end